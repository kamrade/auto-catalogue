...
export let load: PageLoad = async ({ fetch, params }) => {
  ...
  models =
    previousModel !== currentModel
      ? randomBrand
        ? await getModels(fetch, randomBrand.value)
        : currentBrand
  ...
  const randomModel: SelectOption | null =
    currentBrand === "0000000000" || !isBrandValid || !isModelValid ? models[getRandomInt(0, models.length - 1)] : null;
  ...
  const randomGeneration: SelectOption | null =
    currentBrand === "0000000000" || !isBrandValid || !isModelValid || !isGenerationValid
  ...
  const randomModification: SelectOption | null =
    currentBrand === "0000000000" || !isBrandValid || !isModelValid || !isGenerationValid || !isModificationValid
